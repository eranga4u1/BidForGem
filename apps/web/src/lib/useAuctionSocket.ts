...
export interface AuctionSocketHandlers {
  ...
  /** Called on every (re)connect — refetch state so a missed event can't leave the UI stale. */
  onSync?: () => void;
 ...
 * so a dropped socket during a bid war self-heals.
 */
export function useAuctionSocket(auctionId: string | null, handlers: AuctionSocketHandlers): void {
  ...
  useEffect(() => {
    ...
    socket.on("connect", () => {
      ...
      if (auctionId) void socket.emit("join", { auctionId });
      ref.current.onSync?.();
    });
